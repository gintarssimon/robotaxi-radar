import { STATUS, DRIVING, COLORS, dateLabel, isStale, isOverdue, filterRecords, rankRecords, safeURL, drivingLabel, isDrivingStale, isOperating } from './model.js?v=20260924-london';
...
function labelForDate(r) { return r.status === 'announced' ? r.target?.label || 'Termin offen' : r.launchedOn ? dateLabel(r.launchedOn) : isOperating(r) && !r.source.dynamic && r.source.publishedOn ? 'Belegt am '+dateLabel(r.source.publishedOn) : 'Startdatum offen'; }
...
function drawPins(records) {
  ...
  for (const [key, group] of groups) {
    ...
    const pinState=group.some(isOperating)?'operating':group.some(r=>r.status==='testing')?'testing':group.some(r=>r.status==='announced')?'future':'paused';
