...
for(const [provider,config]of Object.entries(SOURCES)){
  const report={pages:0,accepted:0,announcements:0,pendingReview:0,ignored:0,rejected:0,errors:[]};
  try{
    ...
    for(const page of pages){
      ...
      try{
        const news=parseAnnouncements(provider,page,cities,today);issues=news.issues;recognized=news.records.length;
        const candidates=[...parseAvailability(provider,page,cities,today),...news.records];
        const merged=mergeRecords(records,candidates,today);records=merged.records;run.changed+=merged.changed;
        report.accepted+=candidates.length;report.announcements+=recognized;report.ignored+=merged.ignored;
  ...
  console.log(provider+': '+report.pages+' Seiten, '+report.announcements+' Ankündigungen, '+report.errors.length+' Fehler');
}
