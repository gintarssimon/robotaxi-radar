...
function publicationDay($,article,url){
  ...
  if(/^(www\.)?uber\.com$/.test(new URL(url).hostname)&&new URL(url).pathname.includes('/newsroom/')){
    ...
    heading.add(heading.parents().slice(0,3)).each((_,el)=>{
      ...
      if(/^(?:January|February|March|April|May|June|July|August|September|October|November|December) \d{1,2}, 20\d{2}$/.test(label)&&Number.isFinite(Date.parse(label)))values.push(new Date(Date.parse(label)).toISOString());
    });
  }
...
export function parseHTML(html,url,spec,hosts){
  ...
  const publishedOn=isArticle?publicationDay($,article,url):null,links=[];
  $('a[href]').each((_,el)=>{
